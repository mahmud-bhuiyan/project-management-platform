...
async function main() {
  ...
  try {
    ...
    await page.getByTestId('project-detail-task-status-filter').selectOption('BACKLOG');
    await page.waitForTimeout(800);
    log('filter status=BACKLOG', await page.getByText(taskTitle).isVisible());

    await page.getByTestId('project-detail-task-status-filter').selectOption('');
    await page.waitForTimeout(400);

    await page.locator('[data-testid^="project-detail-task-edit-"]').first().click();
